...
def insolation(dates, lat, lon, S=1., daily=False, enforce_2d=False, clip_zero=True):  # pylint: disable=invalid-name
    ...
    if dates[0].dtype=='datetime64[m]': # detect use of lower precision datetime64, used when making extended climate prediction
        start_years = np.array([np.datetime64(f'{dt.astype(object).year}-01-01').astype('datetime64[m]') for dt in dates])
        days_arr = (np.array(dates, dtype='datetime64[m]') - start_years) / np.timedelta64(1, 'D')
    else:
